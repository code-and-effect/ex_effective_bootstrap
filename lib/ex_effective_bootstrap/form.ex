defmodule ExEffectiveBootstrap.Form do
  ...
  defp merge_class(options, classes) do
    Keyword.update(options, :class, Enum.join(classes, " "), fn class -> Enum.join([class] ++ classes, " ") end)
  end
  ...
  defp merge_error(options, %Changeset{valid?: false}), do: merge_class(options, @error)
